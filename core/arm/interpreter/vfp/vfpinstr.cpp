...
void VMSR(ARMul_State * state, ARMword reg, ARMword Rt)
{
	...
	else if (reg == 8 && state->Mode != USER26MODE && state->Mode != USER32MODE)
	{
		DBG("VMSR :\tfpexc <= r%d=[%x]\n", Rt, state->Reg[Rt]);
		state->VFP[VFP_OFFSET(VFP_FPEXC)] = state->Reg[Rt];
	}
	else if (reg == 9 && state->Mode != USER26MODE && state->Mode != USER32MODE)
	{
		DBG("VMSR :\tfpinst <= r%d=[%x]\n", Rt, state->Reg[Rt]);
		state->VFP[VFP_OFFSET(VFP_FPINST)] = state->Reg[Rt];
	}
	else if (reg == 10 && state->Mode != USER26MODE && state->Mode != USER32MODE)
	{
		DBG("VMSR :\tfpinst2 <= r%d=[%x]\n", Rt, state->Reg[Rt]);
		state->VFP[VFP_OFFSET(VFP_FPINST2)] = state->Reg[Rt];
	}
}
#endif
#ifdef VFP_DYNCOM_TABLE
...
{
	state->ExtReg[(2 * CRn) + BIT(instr, 21)] = value;
	return ARMul_DONE;
}
#endif
#ifdef VFP_DYNCOM_TABLE
	...
	if ((inst_base->cond == 0xe) || CondPassed(cpu, inst_base->cond)) {
		...
		else
		{
			...
			case 0:
				...
				break;
			case 6:
				cpu->Reg[inst_cream->Rt] = cpu->VFP[VFP_OFFSET(VFP_MVFR1)];
				DBG("\tr%d <= MVFR1[%08x]\n", inst_cream->Rt, cpu->VFP[VFP_OFFSET(VFP_MVFR1)]);
				break;
			case 7:
				cpu->Reg[inst_cream->Rt] = cpu->VFP[VFP_OFFSET(VFP_MVFR0)];
				DBG("\tr%d <= MVFR0[%08x]\n", inst_cream->Rt, cpu->VFP[VFP_OFFSET(VFP_MVFR0)]);
				break;
			case 8:
				if (cpu->Mode != USER26MODE && cpu->Mode != USER32MODE) {
			...
			case 9:
				if (cpu->Mode != USER26MODE && cpu->Mode != USER32MODE) {
					cpu->Reg[inst_cream->Rt] = cpu->VFP[VFP_OFFSET(VFP_FPINST)];
					DBG("\tr%d <= fpinst[%08x]\n", inst_cream->Rt, cpu->VFP[VFP_OFFSET(VFP_FPINST)]);
				}
				break;
			case 10:
				if (cpu->Mode != USER26MODE && cpu->Mode != USER32MODE) {
					cpu->Reg[inst_cream->Rt] = cpu->VFP[VFP_OFFSET(VFP_FPINST2)];
					DBG("\tr%d <= fpinst2[%08x]\n", inst_cream->Rt, cpu->VFP[VFP_OFFSET(VFP_FPINST2)]);
				}
				break;
			default:
				DBG("\tSUBARCHITECTURE DEFINED\n");
	...
	else
	{
		...
		case 0:
			*value = state->VFP[VFP_OFFSET(VFP_FPSID)];
			...
			break;
		case 6:
			*value = state->VFP[VFP_OFFSET(VFP_MVFR1)];
			DBG("\tr%d <= MVFR1[%08x]\n", Rt, state->VFP[VFP_OFFSET(VFP_MVFR1)]);
			break;
		case 7:
			*value = state->VFP[VFP_OFFSET(VFP_MVFR0)];
			DBG("\tr%d <= MVFR0[%08x]\n", Rt, state->VFP[VFP_OFFSET(VFP_MVFR0)]);
			break;
		case 8:
		...
		case 9:
			if (state->Mode != USER26MODE && state->Mode != USER32MODE) {
				*value = state->VFP[VFP_OFFSET(VFP_FPINST)];
				DBG("\tr%d <= fpinst[%08x]\n", Rt, state->VFP[VFP_OFFSET(VFP_FPINST)]);
			}
			break;
		case 10:
			if (state->Mode != USER26MODE && state->Mode != USER32MODE) {
				*value = state->VFP[VFP_OFFSET(VFP_FPINST2)];
				DBG("\tr%d <= fpinst2[%08x]\n", Rt, state->VFP[VFP_OFFSET(VFP_FPINST2)]);
			}
			break;
		default:
			DBG("\tSUBARCHITECTURE DEFINED\n");
...
if (CoProc == 11 && CRm == 0)
{
	*value = state->ExtReg[(2 * CRn) + BIT(instr, 21)];
	return ARMul_DONE;
}
#endif
#ifdef VFP_DYNCOM_TABLE
...
VFPLABEL_INST:
{
	...
	if ((inst_base->cond == 0xe) || CondPassed(cpu, inst_base->cond)) {
		...
		if (inst_cream->to_arm) {
			cpu->Reg[inst_cream->t] = cpu->ExtReg[inst_cream->m + 0];
			cpu->Reg[inst_cream->t2] = cpu->ExtReg[inst_cream->m + 1];
		} else {
			cpu->ExtReg[inst_cream->m + 0] = cpu->Reg[inst_cream->t];
			cpu->ExtReg[inst_cream->m + 1] = cpu->Reg[inst_cream->t2];
		}
	}
	cpu->Reg[15] += GET_INST_SIZE(cpu);
	INC_PC(sizeof(vfpinstr_inst));
...
{
	VMOVBRRSS(state, 0, Rt, Rt2, (CRm << 1) | BIT(instr, 5), &value1, &value2);
	return ARMul_DONE;
}
#endif
...
if (CoProc == 10 && (OPC_1 & 0xD) == 1)
{
	VMOVBRRSS(state, 1, Rt, Rt2, (CRm << 1) | BIT(instr, 5), value1, value2);
	return ARMul_DONE;
}
#endif
#ifdef VFP_DYNCOM_TABLE
